...
// Función para procesar cada archivo NetCDF y generar CSV por sitio
void process_file(const char *file_path, double target_lat, double target_lon, const char *site_name) {
    ...
    // Crear archivo CSV para el sitio
    char output_filename[256];
    snprintf(output_filename, sizeof(output_filename), "%s_SWTDN.csv", site_name);
...
// Función principal
int main() {
    // Abrir archivo CSV con los sitios
    FILE *sites_file = fopen("sites.csv", "r");
    if (!sites_file) {
        perror("No se puede abrir el archivo CSV de sitios");
        return 1;
    }

    // Leer cada línea del archivo de sitios
    char line[256];
    
    while (fgets(line, sizeof(line), sites_file)) {
        char site_name[100];
        double lat, lon;

        // Leer el nombre del sitio, latitud y longitud
        if (sscanf(line, "%99[^,],%lf,%lf", site_name, &lat, &lon) == 3) {
	...
	printf("\n in site: %s\n", site_name);
            ...
            while ((entry = readdir(dir)) != NULL) {
                ...
                if (strstr(entry->d_name, ".nc4")) {
                    ...
                    process_file(file_path, lat, lon, site_name);
                }
            }
